...
export default {
  ...
  methods: {
    ...
    quickSeat(value) {
      ...
      bestSeatList.reduce(function (prev, cur, index) {
        if (Array.isArray(prev)) {
          // 取中心点离 最好坐标 绝对值
          let n = Math.abs(
            (prev[0].gCol + prev[value - 1].gCol) / 2 - _self.bestX
          );
    ...
    seachBestSeatByRow(rowSeatList, value) {
      ...
      if (
        effectiveSeatLeft.length === value &&
        effectiveSeatRight.length === value
      ) {
        return Math.abs(effectiveSeatLeft[0].gCol - this.bestX) >
          Math.abs(effectiveSeatRight[0].gCol - this.bestX)
          ? effectiveSeatRight
          : effectiveSeatLeft;
      } else {
    ...
    checkSeatMiddle(rowSeatList, value) {
      ...
      for (let j = -activeValue; j <= activeValue; j++) {
        // 确认最佳座位状态
        let iter = rowSeatList.find(
          (el) => parseInt(el.gCol) === this.bestX + j
        );
        // 最佳座位
        if (iter === undefined) {
          break;
    ...
    checkSeat(element, selectedSeat) {
      ...
      // 检查座位左侧
      let left = this.checkSeatDirection(
        gRowBasic,
        gColBasic,
        checkNum,
        "-",
      ...
      // 检查座位右侧
      let right = this.checkSeatDirection(
        gRowBasic,
        gColBasic,
        checkNum,
        "+",
